...
- **three 0.185** (vanilla, no React Three Fiber — its global JSX augmentation breaks the vendored once-ui polymorphic components; the home hero's 3D reference unit lives in `src/components/home/DeviceScene.tsx`, one lazy chunk via `next/dynamic`, loaded on idle ≥1024px only, rendering on the shared ticker), **lenis 1.3** (smooth wheel scroll; `src/components/motion/SmoothScroll.tsx`, off under reduced motion), **framer-motion 12** (Gauge needle spring, `MotionConfig` in `Providers`, and — as the v2 console lands — the scroll rail and nav carriage), **embla-carousel-react 8** (resource carousels)
...
- **Components:** `src/components/console/` — `Panel` (device shell), `TiltPanel` (client Panel that tilts ≤4° toward a fine pointer; device cards only), `Screen` (LCD; statuses: sync = red pulse, live = mint, idle = amber, locked = red steady, off = dotless; glass overlays: lamp specular, vignette, bloom on live/sync, boot sweep), `Gauge`, `Rocker`, `Led`, `MicroLcd`, `Key` (router-aware: internal hrefs render `next/link`; magnetic by default via `data-magnetic`, `magnetic={false}` for track-seated caps; `as="span"` for decorative chips), `Badge`, `Reveal` (scroll reveal: 3D lift off the desk, auto-stagger, sets `data-revealed`), `ScrollRail` (the rack rail: fixed left-edge scroll-progress instrument; sections opt in via `data-rail="LABEL"`; ≥1200px fine pointers), `Screws`/`BootIn` (hardware/per-page boot), `PageTransition` (90ms settle), `DitherCanvasMount` (raw-WebGL desk grain with lamp pool, scroll drift, vignette). Barrel: `@/components/console`. **Runtime layer:** `src/components/motion/` — `ticker.ts` (the one shared rAF loop), `pointer.ts` (pointer store), `PointerLight.tsx` (the desk lamp: writes `--light-dx/dy` on `<html>`; fine pointers ≥1024px, off under reduced motion, `html[data-lighting="static"]` kills it), `useTilt.ts`; `PointerLight` also drives magnetic keys (`--mag-x/--mag-y` on the hovered `[data-magnetic]` cap); `src/components/hooks/useScrolled.ts` feeds the header's condensed `data-scrolled` state; mounted from `src/components/Providers.tsx` (`MotionConfig reducedMotion="user"`). **Home hero dock:** `src/components/home/DeviceDockMount.tsx` (client; loads `DeviceScene` on idle, ≥1024px, non-low-tier) + `DeviceOutline.tsx` (printed SVG fallback/placeholder) + `DeviceDock.module.scss`; the scene reads console tokens and subscribes to the lamp store.
...
- **Mobile:** Lenis syncs touch on coarse pointers (`SmoothScroll.tsx` picks the mode after hydration). Scroll budget: no `perspective()` on panels and no `translateZ` on glass below 1025px/without hover, no header `backdrop-filter` on coarse pointers, `ScrollRail` mounts only on desktop (`useMediaQuery` in `Providers`), dither at ~20fps on touch. `src/styles/console-mobile.scss` (page gutter, `overflow-x: clip`) plus a `@media (max-width: 768px) :root` block in `console-tokens.scss` that steps spacing/radii/readout-xl down; touch sizing lives in `Key.module.scss` (`pointer: coarse`) and `Header.module.scss`. See design.md §4 "Mobile pass".
- **Scroll choreography:** hero recede is a CSS `scroll()` timeline on `.heroShell` (`home.module.scss`); log lines type in via `[data-revealed] .logLine` / `[data-boot="on"] .logLine` with `--i` per line and an optional `--boot-offset`; LEDs flare once inside a revealed shell. All inside `prefers-reduced-motion: no-preference`.
