...
- **Mobile:** `src/styles/console-mobile.scss` (page gutter, `overflow-x: clip`) plus a `@media (max-width: 768px) :root` block in `console-tokens.scss` that steps spacing/radii/readout-xl down; touch sizing lives in `Key.module.scss` (`pointer: coarse`) and `Header.module.scss`. See design.md §4 "Mobile pass".
